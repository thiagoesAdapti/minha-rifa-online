document.addEventListener('DOMContentLoaded', () => {
    ...
    async function carregarCompradores() {
        if (!adminPassword) {
            adminPassword = prompt('Digite a senha de administrador:');
            if (!adminPassword) return; // Se o usuário cancelar, não faz nada
        }
    ...
    async function liberarNumero(numeroId) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.error || 'Senha incorreta ou erro no servidor.');
            }
